...
test.describe("Unmapped Files", () => {
	...
	test("maps an unmapped movie into the managed folder and moves related sidecars", async ({
		...
		await expect(page.getByText("Alien · 1979", { exact: true })).toBeVisible();
	...
	test("maps TV rows with distinct episode targets and preserves nested and show-level related files", async ({
		...
		await expect(
			page.getByText("Search episodes for Severance.S01E01.mkv", {
				exact: true,
			}),
		).toBeVisible();
		await expect(
			page.getByText("Search episodes for Severance.S01E02.mkv", {
				exact: true,
			}),
		).toBeVisible();
		...
		const firstDestPath = join(destinationSeasonDir, "Severance S01E01.mkv");
		const secondDestPath = join(destinationSeasonDir, "Severance S01E02.mkv");
		...
		const folderArtDest = join(
			tempDir,
			`${show.title} (${show.year})`,
			"folder.jpg",
		);
		const themeDest = join(
			tempDir,
			`${show.title} (${show.year})`,
			"theme.mp3",
		);
	...
	test("search and content-type filters narrow mixed unmapped entries", async ({
		...
		await navigateTo(page, appUrl, "/unmapped-files");
		await expect(
			page.getByText(ebookFile.filename, { exact: true }),
		).toBeVisible();
		await expect(
