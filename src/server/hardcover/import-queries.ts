...
/** Parse a single edition record into a HardcoverRawSeriesBookEdition. */
function parseSeriesBookEdition(
	edRec: Record<string, unknown>,
	defaultCoverEditionId: number | null,
): HardcoverRawSeriesBookEdition | undefined {
	...
	return {
		...
		format: mapEditionFormat(
			formatRecord ? (firstString(formatRecord, [["format"]]) ?? null) : null,
		),
...
function parseSeriesBookEntry(
	...
	seen: Set<number>,
): HardcoverRawSeriesBook | undefined {
	...
	const bookEditions = editionRecords
		.map((edRec) => parseSeriesBookEdition(edRec, defaultCoverEditionId))
...
/** Filter out partial editions (fractional-position books that are sub-parts of integer-position books). */
function filterPartialEditions(
	allBooks: HardcoverRawSeriesBook[],
): HardcoverRawSeriesBook[] {
...
export async function fetchSeriesComplete(
	...
	return toRecordArray(data.series)
		.map((s) => {
			...
			const allBooks = bookEntries
				.map((entry) => parseSeriesBookEntry(entry, seen))
				.filter(Boolean) as HardcoverRawSeriesBook[];

			const books = filterPartialEditions(allBooks);
