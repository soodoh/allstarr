...
import { useRefreshSeries, useUpdateSeries } from "src/hooks/mutations/series";
...
import { seriesListQuery } from "src/lib/queries/series";
...
export const Route = createFileRoute("/_authed/series/")({
	loader: async ({ context }) => {
		await Promise.all([
			context.queryClient.ensureQueryData(seriesListQuery()),
...
type SeriesColumnDef = {
	label: string;
	render: (row: SeriesRowData) => ReactNode;
	...
	author: {
		...
		render: (row) =>
			row.bookAuthors.length > 0 ? (
				<AdditionalAuthors bookAuthors={row.bookAuthors} />
			) : (
				row.authorName
			),
		cellClassName: "text-muted-foreground",
...
function SeriesPage() {
	...
	const { data: seriesData } = useSuspenseQuery(seriesListQuery());
	const { data: downloadProfiles } = useSuspenseQuery(
		downloadProfilesListQuery(),
	);
	const { data: metadataProfile } = useSuspenseQuery(metadataProfileQuery());

	const allSeries = seriesData.series as SeriesEntry[];
	const allBooks = seriesData.books as LocalBook[];
	const availableLanguages = seriesData.availableLanguages as LanguageOption[];
	...
	const { data: hardcoverSeries, isLoading: isLoadingSeries } = useQuery({
		...
		enabled: foreignSeriesIds.length > 0,
	});
	...
	const getSeriesEntries = useCallback(
		(s: SeriesEntry): MergedSeriesEntry[] => {
			...
			if (foreignId !== null) {
				...
				for (const hcBook of hcBooks) {
					...
					if ((metadataProfile as MetadataProfile).skipMissingIsbnAsin) {
						qualifyingEditions = qualifyingEditions.filter(
							(e) => e.isbn10 || e.isbn13 || e.asin,
						);
					}
					if ((metadataProfile as MetadataProfile).skipMissingReleaseDate) {
						qualifyingEditions = qualifyingEditions.filter(
							(e) => e.releaseDate,
						);
					}
					...
					if (
						(metadataProfile as MetadataProfile).skipMissingReleaseDate &&
						!hcBook.releaseDate
					) {
						continue;
					}
	...
	const handleToggleMonitored = (s: SeriesEntry) => {
		updateSeries.mutate({ id: s.id, monitored: !s.monitored });
	};
			...
			<div className="space-y-4">
				...
				{seriesWithCounts.length === 0 ? (
					<div className="py-8 text-center text-sm text-muted-foreground">
						{searchQuery
							...
							: "No series with monitored books found."}
					</div>
				) : null}
				...
				{seriesWithCounts.map(({ series: s, entryCount }) => {
					...
					const monitoredCount = s.books.filter(
						(sb) =>
							(bookMap.get(sb.bookId)?.downloadProfileIds?.length ?? 0) > 0,
					).length;
